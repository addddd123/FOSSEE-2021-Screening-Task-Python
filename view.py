        ...
        if btn_state_main=="display":
            ...
            if db_exists==True:
                ...
                if radio_state_dialog.text()=="Beams" and radio_state_dialog.isChecked() == True:
                    rowcount2 = cursor.execute('''SELECT COUNT(*) FROM Beams''').fetchone()[0]+1
                    ...
                    self.export.setStyleSheet("font-size:20px; color:black; background-color:orange;")
                    tblTable.setCellWidget(row1+1, 0,self.export)
                    self.arrow_label=QLabel("  --->>>  ")
                    
                    tblTable.setCellWidget(row1+1, 1,self.arrow_label)
                    file_created=False
                    def export_to_excel():
                        df = read_sql('SELECT * from Beams', conn)
                        s="Beams_csv"+str(datetime.now())[:19]+".csv"
                        df.to_csv(s)
                        self.export_msg=QLabel(self)
                        if not os.path.exists(s):
                            
                            self.export_msg.setText("faied export")
                            tblTable.setCellWidget(row1+1,2,self.export_msg)
                            self.export_msg.setStyleSheet("font:red; background-color:powderblue; font-size:20px;")
                        else:
                            self.export_msg.setText("export success")
                            tblTable.setCellWidget(row1+1,2,self.export_msg)
                            self.export_msg.setStyleSheet("font:black; background-color:powderblue; font-size:15px;")
                    self.export.clicked.connect(export_to_excel)
                    ...
                    rowcount_ = cursor.execute('''SELECT COUNT(*) FROM Channels''').fetchone()[0]+1
                    ...
                    self.export=QPushButton("export")
                    self.export.setStyleSheet("font-size:20px; color:black; background-color:orange;")
                    tblTable.setCellWidget(row2+1, 0,self.export)
                    self.arrow_label=QLabel("  --->>>  ")
                    
                    tblTable.setCellWidget(row2+1, 1,self.arrow_label)
                    file_created=False
                    def export_to_excel():
                        df = read_sql('SELECT * from Channels', conn)
                        s="Channels_csv "+str(datetime.now())[:19]+".csv"
                        df.to_csv(s)
                        self.export_msg=QLabel(self)
                        if not os.path.exists(s):
                            
                            self.export_msg.setText("faied export")
                            tblTable.setCellWidget(row2+1,2,self.export_msg)
                            self.export_msg.setStyleSheet("font:red; background-color:powderblue; font-size:20px;")
                        else:
                            self.export_msg.setText("export success")
                            tblTable.setCellWidget(row2+1,2,self.export_msg)
                    ...
                    rowcount3 = cursor.execute('''SELECT COUNT(*) FROM Angles ''').fetchone()[0]+1
                    ...
                    self.export=QPushButton("export")
                    self.export.setStyleSheet("font-size:20px; color:black; background-color:orange;")
                    tblTable.setCellWidget(row+1, 0,self.export)
                    self.arrow_label=QLabel("  --->>>  ")
                    
                    tblTable.setCellWidget(row+1, 1,self.arrow_label)
                    file_created=False
                    def export_to_excel():
                        df = read_sql('SELECT * from Angles', conn)
                        s="Angles_csv "+str(datetime.now())[:19]+".csv"
                        df.to_csv(s)
                        self.export_msg=QLabel(self)
                        if not os.path.exists(s):
                            
                            self.export_msg.setText("faied export")
                            tblTable.setCellWidget(row+1,2,self.export_msg)
                            self.export_msg.setStyleSheet("font:red; background-color:powderblue; font-size:20px;")
                        else:
                            self.export_msg.setText("export success")
                            tblTable.setCellWidget(row+1,2,self.export_msg)
    ...
    ###############Appending code starts below##################################
            ...
            def make_dialog_for_channel_or_beams(self,radio_state_know):
                    ...
                    if radio_state_know=="Channels":
                            ...
                            try:
                                ...
                                if get[0]=='' and get[19]=='':
                                    ...
                                    return  value_warn("enter proper value in Source")
                                elif get[0]=='' :
                                    
                                    return  value_warn("enter proper value in Designation ")
                                                                        
                                #
                            except:
                                return value_warn("""enter proper values\n follow info from placeholders """)
                ...
                def retranslateUi1(self, Dialog1):
                    ...
                    if radio_state_know=="Channels":
                        self.channel_Designation.setPlaceholderText(_translate("Dialog", "Varchar(50)"))
                        self.channel_Mass.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.Area.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_D.setPlaceholderText(_translate("Dialog", "INTEGER(50)"))
                        self.channel_B.setPlaceholderText(_translate("Dialog", "INTEGER(50)"))
                        self.channel_tw.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.lineEdit_15.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.lineEdit_16.setPlaceholderText(_translate("Dialog", "INTEGER"))
                        self.channel_R1.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_R2.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_Iz.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_Iy.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_rz.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_ry.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_zz.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_Zy.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_Zpz.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_Zpy.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_Source.setPlaceholderText(_translate("Dialog", "VARCHAR(100"))
                        self.channel_Cy.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    # self.channel_Source_3.setPlaceholderText(_translate("Dialog", "VARCHAR(100)")) 
                    else:
                        self.channel_Designation.setPlaceholderText(_translate("Dialog", "Varchar(50)"))
                        self.channel_Mass.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.Area.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_D.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_B.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.channel_tw.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.lineEdit_15.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                        self.lineEdit_16.setPlaceholderText(_translate("Dialog", "INTEGER"))
                        self.channel_R1.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
            ...
            def append_data_into_angels_dialog(self):
                ...
                self.Area.setStyleSheet("background-color: rgb(255, 255, 255);")        
                self.Area.setObjectName("Area")
                self.formLayout.setWidget(5, QtWidgets.QFormLayout.FieldRole, self.Area)##
## 
                self.label_28_ = QtWidgets.QLabel("Area")
                self.label_28_.setObjectName("label_28_")
                self.formLayout.setWidget(3, QtWidgets.QFormLayout.LabelRole, self.label_28_)
                self.angle_area_ = QtWidgets.QLineEdit(self.formLayoutWidget)
                self.angle_area_.setStyleSheet("background-color: rgb(255, 255, 255);")
                self.angle_area_.setObjectName("angle_Iz")
                self.formLayout.setWidget(3, QtWidgets.QFormLayout.FieldRole, self.angle_area_)
               
                                
 ## 
## 
                self.label_29_ = QtWidgets.QLabel("AxB")
                self.label_29_.setObjectName("label_29_")
                self.formLayout.setWidget(4, QtWidgets.QFormLayout.LabelRole, self.label_29_)
                self.angle_axb = QtWidgets.QLineEdit(self.formLayoutWidget)
                self.angle_axb.setStyleSheet("background-color: rgb(255, 255, 255);")
                self.angle_axb.setObjectName("angle_Iz")
                self.formLayout.setWidget(4, QtWidgets.QFormLayout.FieldRole, self.angle_axb)
 ...
 ##               
                ...
                self.formLayout.setWidget(6, QtWidgets.QFormLayout.FieldRole, self.angle_D)
                self.label_15 = QtWidgets.QLabel(self.formLayoutWidget)
                self.label_15.setObjectName("label_15")
                ...
                self.formLayout.setWidget(7, QtWidgets.QFormLayout.FieldRole, self.angle_B)
                self.label_14 = QtWidgets.QLabel(self.formLayoutWidget)
                self.label_14.setObjectName("label_14")
                ...
                self.formLayout.setWidget(8, QtWidgets.QFormLayout.FieldRole, self.angle_tw)
                ...
                self.formLayout.setWidget(9, QtWidgets.QFormLayout.LabelRole, self.angle_T)
                self.lineEdit_15 = QtWidgets.QLineEdit(self.formLayoutWidget)
                self.lineEdit_15.setStyleSheet("background-color: rgb(255, 255, 255);\n"
                "")
                self.lineEdit_15.setObjectName("lineEdit_15")
                ...
                self.formLayout.setWidget(10, QtWidgets.QFormLayout.LabelRole, self.angle_FlangeSlope)
                self.lineEdit_16 = QtWidgets.QLineEdit(self.formLayoutWidget)
                self.lineEdit_16.setStyleSheet("background-color: rgb(255, 255, 255);")
                self.lineEdit_16.setObjectName("lineEdit_16")
                ...
                self.formLayout.setWidget(11, QtWidgets.QFormLayout.FieldRole, self.angle_R1)
                self.label_27 = QtWidgets.QLabel(self.formLayoutWidget)
                self.label_27.setObjectName("label_27")
                ...
                self.formLayout.setWidget(12, QtWidgets.QFormLayout.FieldRole, self.angle_R2)
                ...
                self.formLayout_2.setWidget(1, QtWidgets.QFormLayout.FieldRole, self.angle_Iz)
                self.label_37 = QtWidgets.QLabel(Dialog1)
                self.label_37.setGeometry(QRect(20, 10, 241, 20))
                self.label_37.setStyleSheet("color: rgb(0, 170, 0);\n"
                "font: 75 12pt \"MS Shell Dlg 2\";")
                ...
                def get_data_from_Angle_form(get):
                            ...
                            try:
                                ...
                                return value_warn1("enter proper values\nfollow info from placeholders") 
                ...
                self.angle_push_submit.clicked.connect(lambda:get_data_from_Angle_form([self.angle_Designation.text(),
                                                                    ...
                                                                    self.angle_area_.text(),self.angle_axb.text(),self.Area.text()
                                                                    
                                                                   ,self.angle_D.text(),self.angle_B.text()
                                                                   ,self.angle_tw.text(),self.lineEdit_15.text()
                ...
                def retranslateUi2(self, Dialog1):
                    ...
                    self.angle_Designation.setPlaceholderText(_translate("Dialog", "Varchar(50)"))
                    self.angle_Mass.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_area_.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_axb.setPlaceholderText(_translate("Dialog", "VARCHAR"))
                    self.Area.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_D.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_B.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_tw.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.lineEdit_15.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.lineEdit_16.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_R1.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_R2.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_Iz.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_Iy.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_rz.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_ry.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_zz.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_Zy.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_Zpz.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_Zpy.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_Source.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_Source_2.setPlaceholderText(_translate("Dialog", "Real(10,2)"))
                    self.angle_Source_3.setPlaceholderText(_translate("Dialog", "VARCHAR(100)"))
                    # self..setPlaceholderText(_translate("Dialog", "VARCHAR(100)"))
